...
async function main() {
  ...
  await logTx(VeFnxDistributorUpgradeable, VeFnxDistributorUpgradeable.grantRole(ethers.id('DISTRIBUTOR_ROLE'), deployer.address));

  await logTx(VeFnxDistributorUpgradeable, VeFnxDistributorUpgradeable.grantRole(ethers.id('WITHDRAWER_ROLE'), deployer.address));
}
