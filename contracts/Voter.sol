...
contract Voter is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _reset(uint256 _tokenId) internal {
        ...
        for (uint256 i = 0; i < _poolVoteCnt; i++) {
            ...
            if (_votes != 0) {
                ...
                IBribeUpgradeable(internal_bribes[gauges[_pool]]).withdraw(uint256(_votes), _tokenId);
                IBribeUpgradeable(external_bribes[gauges[_pool]]).withdraw(uint256(_votes), _tokenId);
    ...
    function _vote(uint256 _tokenId, address[] memory _poolVote, uint256[] memory _weights) internal {
        ...
        for (uint256 i = 0; i < _poolCnt; i++) {
            ...
            if (isGauge[_gauge] && isAlive[_gauge]) {
                ...
                IBribeUpgradeable(internal_bribes[_gauge]).deposit(uint256(_poolWeight), _tokenId);
                IBribeUpgradeable(external_bribes[_gauge]).deposit(uint256(_poolWeight), _tokenId);
    ...
    function claimBribes(address[] memory _bribes, address[][] memory _tokens, uint256 _tokenId) external {
        ...
        for (uint256 i = 0; i < _bribes.length; i++) {
            IBribeUpgradeable(_bribes[i]).getRewardForOwner(_tokenId, _tokens[i]);
        }
    }
    ...
    function claimFees(address[] memory _fees, address[][] memory _tokens, uint256 _tokenId) external {
        ...
        for (uint256 i = 0; i < _fees.length; i++) {
            IBribeUpgradeable(_fees[i]).getRewardForOwner(_tokenId, _tokens[i]);
        }
    }
    ...
    function _createGauge(
        ...
        address _owner = IPermissionsRegistry(permissionRegistry).fenixTeamMultisign();
        string memory _type = string.concat("Fenix LP Fees: ", IERC20Metadata(_pool).symbol());
        _internal_bribe = IBribeFactoryUpgradeable(bribefactory).createBribe(_owner, tokenA, tokenB, _type);

        _type = string.concat("Fenix Bribes: ", IERC20Metadata(_pool).symbol());
        _external_bribe = IBribeFactoryUpgradeable(bribefactory).createBribe(_owner, tokenA, tokenB, _type);

        // create gauge
        _gauge = ICLGaugeFactoryUpgradeable(_gaugeFactory).createGauge(
            base,
            _ve,
            _pool,
            address(this),
    ...
    function _epochTimestamp() public view returns (uint256) {
        return IEmissionManagerUpgradeable(minter).activePeriod();
    }
    ...
    function distributeAll() external nonReentrant {
        IEmissionManagerUpgradeable(minter).updatePeriod();
