 ...
 */
contract EmissionManagerUpgradeable is IEmissionManagerUpgradeable, Ownable2StepUpgradeable { // @risk centralized risks
     ...
     */
    function initialize( // @risk front running
        IVoter voter_,
        IVotingEscrow votingEscrow_,
        IRewardsDistributor rewardsDistributor_
    ) external initializer override virtual {
        __Ownable2Step_init();
    ...
    function initialize(
        ...
        uint256 max
    ) external reinitializer(2) override virtual onlyOwner {
        if(max > 0){
    ...
    function setVoter(address voter_) external onlyOwner override virtual {
        if (voter_ == address(0)) {
            revert ZeroAddress();
        }
        voter = IVoter(voter_);
    ...
    function setTeamRate(uint256 teamRate_) external onlyOwner override virtual {
    ...
    function setEmission(uint256 emissionRate_) external onlyOwner override virtual {
    ...
    function setRebase(uint256 rebase_) external onlyOwner override virtual {
    ...
    function updatePeriod() external override virtual returns (uint256) {
    ...
    function circulatingEmission() external view override virtual returns (uint256) {
        return _circulatingSupply();
    }

    // calculate inflation and adjust ve balances accordingly
    function calculateReabase(uint256 weeklyMint_) external view override virtual returns (uint256) {
        return _calculateReabase(weeklyMint_);
    }

    // calculate circulating supply as total token supply - locked supply
    function circulatingSupply() external view override virtual returns (uint256) {
        return _circulatingSupply();
    }

    // emission calculation is 1% of available supply to mint adjusted by circulating / total supply
    function calculateEmission() external view override virtual returns (uint256) {
        return _calculateEmission();
    }

    // weekly emission takes the max of calculated (aka target) emission versus circulating tail end emission
    function weeklyEmmision() external view override virtual returns (uint256) {
        return _weeklyEmmision();
    }

    function check() external view override virtual returns(bool) {
        return (activePeriod >= activePeriod + PERIOD_DURATION && _getInitializedVersion() > 1);
    }

    function period() external view override virtual returns(uint256) {
        return _period();
    }
