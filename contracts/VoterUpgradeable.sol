...
contract VoterUpgradeable is IVoterUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => bool) public factoryForGaugeTypeIsAdded;
    ...
    {
        require(!factoryForGaugeTypeIsAdded[gaugeType_.pairFactory], "fact");
        require(!factoryForGaugeTypeIsAdded[gaugeType_.gaugeFactory], "gFact");
    ...
    function removeFactory(uint256 pos_) external virtual override onlyVoterAdmin {
        ...
        require(factoryForGaugeTypeIsAdded[gaugeType.pairFactory], "!fact");
        require(factoryForGaugeTypeIsAdded[gaugeType.gaugeFactory], "!gFact");
    ...
    function killGauge(address gauge_) external virtual override onlyGovernance {
        ...
        totalWeightsPerEpoch[_time] -= weightsPerEpoch[_time][gaugesState[gauge_].pool];

        emit GaugeKilled(gauge_);
    }

    function reviveGauge(address gauge_) external virtual override onlyGovernance {
        require(!gaugesState[gauge_].isAlive, "alive");
        require(gaugesState[gauge_].isGauge, "killed");
    ...
    function claimRewards(address[] calldata gauge_s) external virtual override {
        for (uint256 i = 0; i < gauge_s.length; i++) {
            ICLGaugeUpgradeable(gauge_s[i]).getReward(msg.sender);
        }
    }
    ...
    function claimBribes(
        ...
        for (uint256 i; i < bribes_.length; ) {
            IBribeUpgradeable(bribes_[i]).getRewardForOwner(tokenId_, tokens_[i]);
            unchecked {
                i++;
            }
        }
    ...
    function claimFees(
        ...
        for (uint256 i; i < fees_.length; ) {
            IBribeUpgradeable(fees_[i]).getRewardForOwner(tokenId_, tokens_[i]);
            unchecked {
                i++;
            }
        }
    ...
    function distributeAll() external virtual override nonReentrant {
        ...
        for (uint256 i; i < pools.length; ) {
            _distribute(poolForGauge[pools[i]]);
            unchecked {
                i++;
            }
        }
    ...
    function isGauge(address gauge_) public view virtual override returns (bool) {
        return gaugesState[gauge_].isGauge;
    }

    function gaugeByPair(address gauge_) public view virtual override returns (bool) {
        return gaugesState[gauge_].isGauge;
    }

    function internalBribe(address gauge_) public view virtual override returns (address) {
        return gaugesState[gauge_].internalBribe;
    }

    function externalBribe(address gauge_) public view virtual override returns (address) {
    ...
    function weights(address _pool) public view virtual override returns (uint256) {
        uint256 _time = _epochTimestamp();
        return weightsPerEpoch[_time][_pool];
    }

    function weightsAt(address _pool, uint256 _time) external view virtual override returns (uint256) {
        return weightsPerEpoch[_time][_pool];
    }

    function totalWeight() public view virtual override returns (uint256) {
        uint256 _time = _epochTimestamp();
        return totalWeightsPerEpoch[_time];
    }

    function totalWeightAt(uint256 _time) public view virtual override returns (uint256) {
        return totalWeightsPerEpoch[_time];
    }
    ...
    function _createGauge(
        ...
        require(gaugeTypeIndex_ < gaugeTypes.length, "gaugetype");
        require(poolForGauge[pool_] == address(0x0), "!exists");

        bool isPair;
        GaugeType memory gaugeType = gaugeTypes[gaugeTypeIndex_];

        require(gaugeType.pairFactory != address(0), "addr0");
        require(gaugeType.gaugeFactory != address(0), "addr0");
        ...
        poolForGauge[pool_] = deployedGauge;
        pools.push(pool_);
    ...
    function _vote(uint256 _tokenId, address[] memory _poolVote, uint256[] memory _weights) internal {
        ...
        for (uint256 i = 0; i < _poolCnt; i++) {
            if (gaugesState[poolForGauge[_poolVote[i]]].isAlive) _totalVoteWeight += _weights[i];
        }

        for (uint256 i = 0; i < _poolCnt; i++) {
            address _pool = _poolVote[i];
            address gauge_ = poolForGauge[_pool];
    ...
    function _reset(uint256 tokenId_) internal {
        address[] storage _poolVote = poolVote[tokenId_];
        ...
        for (uint256 i = 0; i < _poolVoteCnt; i++) {
            ...
            if (_votes != 0) {
                ...
                address gauge = poolForGauge[_pool];
