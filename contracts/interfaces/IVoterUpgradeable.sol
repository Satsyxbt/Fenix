...
interface IVoterUpgradeable {
    ...
    error PairFactoryNotExist();
    error GaugeFactoryNotExist();
    ...
    function factoryForGaugeTypeIsAdded(address factory_) external view returns (bool);

    function isWhitelisted(address token_) external view returns (bool);

    function weights(address _pool) external view returns (uint256);

    function weightsPerEpoch(uint256 time_, address pool_) external view returns (uint256);

    function totalWeight() external view returns (uint256);

    function totalWeightsPerEpoch(uint256 time_) external view returns (uint256);
    ...
    function gaugeByPool(address pool_) external view returns (address);
