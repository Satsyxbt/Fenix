...
contract BribeUpgradeable is IBribeUpgradeable, ReentrancyGuard {
    ...
    function totalSupply() external view returns (uint256) {
        uint256 _currentEpochStart = IEmissionManagerUpgradeable(minter).activePeriod(); // claim until current epoch
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward) external nonReentrant {
        ...
        uint256 _startTimestamp = IEmissionManagerUpgradeable(minter).activePeriod() + WEEK; //period points to the current thursday. Bribes are distributed from next epoch (thursday)
