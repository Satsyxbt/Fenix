...
contract OptionToken is ERC20, AccessControl {
    ...
    event SetTwapOracleAndPaymentToken(IUniswapV3Twap indexed _twapOracle, address indexed _paymentToken);
    ...
    /// @notice The voting escrow for locking FNX to veFNX
    ...
    /// @notice the further discount for locking to veFNX
    ...
    /// @dev A modifier which checks that the caller has the admin role.
    modifier onlyMinter() {
        if (!hasRole(ADMIN_ROLE, msg.sender) && !hasRole(MINTER_ROLE, msg.sender)) revert OptionToken_NoMinterRole();
        _;
    }
    ...
    function getTimeWeightedAveragePrice(uint256 _amount) public view returns (uint256) {
        return twapOracle.estimateAmountOut(address(underlyingToken), uint128(_amount), twapSeconds);
    }
    ...
    function setTwapOracleAndPaymentToken(IUniswapV3Twap _twapOracle, address _paymentToken) external onlyAdmin {
        if (
            !((_twapOracle.token0() == _paymentToken && _twapOracle.token1() == address(underlyingToken)) ||
                (_twapOracle.token0() == address(underlyingToken) && _twapOracle.token1() == _paymentToken))
    ...
    function _exercise(uint256 _amount, uint256 _maxPaymentAmount, address _recipient) internal returns (uint256 paymentAmount) {
