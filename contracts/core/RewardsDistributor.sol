...
import "./interfaces/IVotingEscrowUpgradeable.sol";
...
contract RewardsDistributor is IRewardsDistributor {
    ...
    constructor(address _voting_escrow) {
        ...
        last_token_time = _t;
        time_cursor = _t;
        address _token = IVotingEscrowUpgradeable(_voting_escrow).token();
        token = _token;
    ...
    function _find_timestamp_epoch(address ve, uint _timestamp) internal view returns (uint) {
        uint _min = 0;
        uint _max = IVotingEscrowUpgradeable(ve).epoch();
        for (uint i = 0; i < 128; i++) {
            if (_min >= _max) break;
            uint _mid = (_min + _max + 2) / 2;
            IVotingEscrowUpgradeable.Point memory pt = IVotingEscrowUpgradeable(ve).point_history(_mid);
            if (pt.ts <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid - 1;
    ...
    function _find_timestamp_user_epoch(address ve, uint tokenId, uint _timestamp, uint max_user_epoch) internal view returns (uint) {
        ...
        for (uint i = 0; i < 128; i++) {
            if (_min >= _max) break;
            uint _mid = (_min + _max + 2) / 2;
            IVotingEscrowUpgradeable.Point memory pt = IVotingEscrowUpgradeable(ve).user_point_history(tokenId, _mid);
            if (pt.ts <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid - 1;
    ...
    function ve_for_at(uint _tokenId, uint _timestamp) external view returns (uint) {
        ...
        uint max_user_epoch = IVotingEscrowUpgradeable(ve).user_point_epoch(_tokenId);
        uint epoch = _find_timestamp_user_epoch(ve, _tokenId, _timestamp, max_user_epoch);
        IVotingEscrowUpgradeable.Point memory pt = IVotingEscrowUpgradeable(ve).user_point_history(_tokenId, epoch);
    ...
    function _checkpoint_total_supply() internal {
        ...
        IVotingEscrowUpgradeable(ve).checkpoint();
        ...
        for (uint i = 0; i < 20; i++) {
            if (t > rounded_timestamp) {
                ...
                IVotingEscrowUpgradeable.Point memory pt = IVotingEscrowUpgradeable(ve).point_history(epoch);
                int128 dt = 0;
                if (t > pt.ts) {
    ...
    function _claim(uint _tokenId, address ve, uint _last_token_time) internal returns (uint) {
        ...
        IVotingEscrowUpgradeable.Point memory user_point = IVotingEscrowUpgradeable(ve).user_point_history(_tokenId, user_epoch);

        if (week_cursor == 0) week_cursor = ((user_point.ts + WEEK - 1) / WEEK) * WEEK;
        if (week_cursor >= last_token_time) return 0;
        if (week_cursor < _start_time) week_cursor = _start_time;

        IVotingEscrowUpgradeable.Point memory old_user_point;
        ...
        for (uint i = 0; i < 50; i++) {
            ...
            if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
                ...
                if (user_epoch > max_user_epoch) {
                    user_point = IVotingEscrowUpgradeable.Point(0, 0, 0, 0);
                } else {
                    user_point = IVotingEscrowUpgradeable(ve).user_point_history(_tokenId, user_epoch);
                }
            } else {
    ...
    function claim(uint _tokenId) external returns (uint) {
        ...
        if (amount != 0) {
            ...
            IVotingEscrowUpgradeable.LockedBalance memory _locked = IVotingEscrowUpgradeable(voting_escrow).locked(_tokenId);
            if (_locked.end < block.timestamp) {
                address _nftOwner = IVotingEscrowUpgradeable(voting_escrow).ownerOf(_tokenId);
                IERC20(token).transfer(_nftOwner, amount);
            } else {
                IVotingEscrowUpgradeable(voting_escrow).deposit_for(_tokenId, amount);
            }
    ...
    function claim_many(uint[] memory _tokenIds) external returns (bool) {
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            ...
            if (amount != 0) {
                // if locked.end then send directly
                IVotingEscrowUpgradeable.LockedBalance memory _locked = IVotingEscrowUpgradeable(_voting_escrow).locked(_tokenId);
                if (_locked.end < block.timestamp) {
                    address _nftOwner = IVotingEscrowUpgradeable(_voting_escrow).ownerOf(_tokenId);
                    IERC20(token).transfer(_nftOwner, amount);
                } else {
                    IVotingEscrowUpgradeable(_voting_escrow).deposit_for(_tokenId, amount);
                }
                total += amount;
            }
        }
