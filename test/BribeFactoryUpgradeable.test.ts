...
describe('BribeFactoryUpgradeable Contract', function () {
  ...
  beforeEach(async function () {
    ...
    defaultRewardsToken = [
        await result.tokens.token18.getAddress(),
        await result.tokens.token9.getAddress(),
    ]

    await bribeFactoryProxy.initialize(
        voter.address,
        await bribeFactoryImplementation.getAddress(),
        defaultRewardsToken
    );
  });
  ...
  describe('Deployment', function () {
    it('Initialization should be disable on implementaion', async function () {
        await expect(bribeFactoryImplementation.initialize(
    ...
    it('Should correct set initial settings after initialization', async function () {
        ...
        expect(await bribeFactoryProxy.implementation())
        .to.be.equal(await bribeFactoryImplementation.getAddress())
    ...
    it('Should fail initialization if array of defaultRewardToken have any zero addreses', async function () {
        await expect(bribeFactoryProxy.initialize(
            voter.address,
            await bribeFactoryImplementation.getAddress(),
            [ZERO_ADDRESS, defaultRewardsToken[0]]
    ...
    it('Should fail initialization if array of defaultRewardToken have token address duplicate', async function () {
        await expect(bribeFactoryProxy.initialize(
            voter.address,
            await bribeFactoryImplementation.getAddress(),
            [defaultRewardsToken[0], defaultRewardsToken[0]]
