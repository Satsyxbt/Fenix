  ...
  describe('Create bribe', async function () {
    ...
    describe('Should corectly create new bribe', async function () {
      ...
      it('Should corect initialize deployead bribe', async function () {
        ...
        expect(await bribe.rewardTokens(0)).to.be.equal(await token6.getAddress());
        expect(await bribe.rewardTokens(1)).to.be.equal(await token5.getAddress());

        expect(await bribe.rewardTokens(2)).to.be.equal(await token18.getAddress());
        expect(await bribe.rewardTokens(3)).to.be.equal(await token9.getAddress());
        expect(await bribe.rewardsListLength()).to.be.equal(4);
      });
    ...
    describe('Should corectly create bribe without token1 and token2', async function () {
      ...
      it('Should corect initialize deployead bribe', async function () {
        ...
        expect(await bribe.rewardTokens(0)).to.be.equal(await token18.getAddress());
        expect(await bribe.rewardTokens(1)).to.be.equal(await token9.getAddress());
        expect(await bribe.rewardsListLength()).to.be.equal(2);
      });
    });
  });
  describe('Functions', async function () {
    describe('#pushDefaultRewardToken', async function () {
      ...
      it('Should corectly add new token and start added to future bribes', async function () {
        ...
        expect(await bribe.rewardTokens(0)).to.be.equal(await token18.getAddress());
        expect(await bribe.rewardTokens(1)).to.be.equal(await token9.getAddress());
        expect(await bribe.rewardTokens(2)).to.be.equal(await token5.getAddress());
        expect(await bribeFactoryUpgradeableProxy.getDefaultRewardTokens()).to.be.deep.equal([
          await token18.getAddress(),
          await token9.getAddress(),
          await token5.getAddress(),
    ...
    describe('#removeDefaultRewardToken', async function () {
      ...
      it('Should corectly remove token and start skip for future bribes', async function () {
        ...
        expect(await bribe.rewardTokens(0)).to.be.equal(await token9.getAddress());
    ...
    describe('#setBribesMinter', async function () {
      it('Should corectly set new minter owner for list of bribes', async function () {
        ...
        let bribe4 = await createBribe(fenixMultisig.address, ZERO_ADDRESS, ZERO_ADDRESS, 'type 4');
        let bribe5 = await createBribe(fenixMultisig.address, ZERO_ADDRESS, ZERO_ADDRESS, 'type 5');
        expect(await bribe.minter()).to.be.equal(minter.address);
        expect(await bribe2.minter()).to.be.equal(minter.address);
        expect(await bribe3.minter()).to.be.equal(minter.address);
        expect(await bribe4.minter()).to.be.equal(minter.address);
        expect(await bribe5.minter()).to.be.equal(minter.address);

        await bribeFactoryUpgradeableProxy.setBribesMinter([await bribe3.getAddress(), await bribe5.getAddress()], deployer.address);

        expect(await bribe.minter()).to.be.equal(minter.address);
        expect(await bribe2.minter()).to.be.equal(minter.address);
        expect(await bribe3.minter()).to.be.equal(deployer.address);
        expect(await bribe4.minter()).to.be.equal(minter.address);
        expect(await bribe5.minter()).to.be.equal(deployer.address);
      });
    });
    ...
    describe('#addRewardTokensToBribes', async function () {
      ...
      it('Should corectly add reward tokens to bribes', async function () {
        ...
        let bribe2 = await createBribe(fenixMultisig.address, ZERO_ADDRESS, ZERO_ADDRESS, 'type 2');
        let bribe3 = await createBribe(fenixMultisig.address, ZERO_ADDRESS, ZERO_ADDRESS, 'type 3');
        await expect(bribe1.rewardTokens(0)).to.be.reverted;
        await expect(bribe2.rewardTokens(0)).to.be.reverted;
        await expect(bribe3.rewardTokens(0)).to.be.reverted;
        ...
        expect(await bribe1.rewardTokens(0)).to.be.equal(await token18.getAddress());
        expect(await bribe1.rewardTokens(1)).to.be.equal(await token5.getAddress());
        await expect(bribe3.rewardTokens(2)).to.be.reverted;

        await expect(bribe2.rewardTokens(0)).to.be.reverted;

        expect(await bribe3.rewardTokens(0)).to.be.equal(await token9.getAddress());
        await expect(bribe3.rewardTokens(1)).to.be.reverted;
      });
    });
    ...
    describe('#addRewardTokenToBribes', async function () {
      it('Should corectly add reward token to bribes', async function () {
        ...
        expect(await bribe1.rewardTokens(0)).to.be.equal(await token18.getAddress());
        expect(await bribe2.rewardTokens(0)).to.be.equal(await token18.getAddress());
      });
    });
    ...
    describe('#recoverERC20AndUpdateData', async function () {
      ...
      it('Should corectly transfer ERC20 tokens from bribe to recipient', async function () {
        ...
        await bribeFactoryUpgradeableProxy.setBribesMinter(
          [await bribe1.getAddress(), await bribe2.getAddress()],
