...
describe('TickLens', () => {
  ...
  describe('#getPopulatedTicksInWord', () => {
    ...
    it('works for min/max and -2/-1/0/1', async () => {
      ...
      const [one, zero] = await tickLens.getPopulatedTicksInWord(poolAddress, getTickBitmapIndex(plus, TICK_SPACINGS[FeeAmount.MEDIUM]));
    ...
    it('fully populated ticks', async () => {
      ...
      const ticks = await tickLens.getPopulatedTicksInWord(poolAddress, getTickBitmapIndex(0, TICK_SPACINGS[FeeAmount.MEDIUM]));
      expect(ticks.length).to.be.eq(256);

      await snapshotGasCost(
        tickLens.getGasCostOfGetPopulatedTicksInWord(poolAddress, getTickBitmapIndex(0, TICK_SPACINGS[FeeAmount.MEDIUM])),
      );
    }).timeout(300_000);
  });
});
