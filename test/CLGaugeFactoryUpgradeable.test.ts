...
import { voterSol } from '../typechain-types/contracts';
  ...
  describe('Functions', async function () {
    ...
    describe('#activateEmergencyMode', async function () {
      it('Should corectly active emergency mode on gauges', async function () {
        ...
        expect(await g1.emergency()).to.be.false;
        expect(await g2.emergency()).to.be.false;
        expect(await g3.emergency()).to.be.false;
        ...
        expect(await g1.emergency()).to.be.true;
        expect(await g2.emergency()).to.be.false;
        expect(await g3.emergency()).to.be.true;
      });
    });
    describe('#stopEmergencyMode', async function () {
      it('Should corectly stop emergency mode on gauges', async function () {
        ...
        expect(await g1.emergency()).to.be.true;
        expect(await g2.emergency()).to.be.true;
        expect(await g3.emergency()).to.be.true;
    ...
    describe('#setInternalBribe', async function () {
      ...
      it('Should corectly update intenral bribe on gauges', async function () {
        ...
        expect(await g1.internal_bribe()).to.be.equal(internalBribe.address);
        expect(await g2.internal_bribe()).to.be.equal(internalBribe.address);
        expect(await g3.internal_bribe()).to.be.equal(internalBribe.address);
        ...
        expect(await g1.internal_bribe()).to.be.equal(internalBribe.address);
        expect(await g2.internal_bribe()).to.be.equal(extenralBribe.address);
        expect(await g3.internal_bribe()).to.be.equal(extenralBribe.address);
      });
    });
  ...
  describe('Create gauge', async function () {
    describe('Should corectly create new gauge and CLFeesVault', async function () {
      ...
      it('Should corect emit event after deploy new gauge', async function () {
        await expect(deployedTx).to.emit(clGaugeFactoryUpgradeableProxy, 'GaugeCreated').withArgs(deployedGaugeAddress, gaugeFeeVault);
      });
      ...
      it('Should corect initialize deployead gauge', async function () {
        ...
        expect(await gauge._VE()).to.be.equal(votingEscrow.address);
        expect(await gauge.rewardToken()).to.be.equal(await token18.getAddress());
        expect(await gauge.TOKEN()).to.be.equal(await token9.getAddress());
        expect(await gauge.DISTRIBUTION()).to.be.equal(await voteMock.getAddress());
        expect(await gauge.internal_bribe()).to.be.equal(internalBribe.address);
        expect(await gauge.external_bribe()).to.be.equal(extenralBribe.address);
