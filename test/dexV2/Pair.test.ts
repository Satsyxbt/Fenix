...
describe('Pair Contract', function () {
  ...
  beforeEach(async function () {
    ...
    pairStable = await ethers.getContractAt('Pair', await pairFactory.getPair(deployed.solex.target, tokenTK6.target, true));
  ...
  describe('deployments', async () => {
    ...
    it('corect initialize start parameters', async () => {
      ...
      if (deployed.solex.target.toString().toLowerCase() < tokenTK6.target.toString().toLowerCase()) {
        expect(await pairStable.name()).to.be.eq('StableV1 AMM - SOLEX/TK6');
        expect(await pairStable.symbol()).to.be.eq('sAMM-SOLEX/TK6');
      } else {
        expect(await pairStable.name()).to.be.eq('StableV1 AMM - TK6/SOLEX');
        expect(await pairStable.symbol()).to.be.eq('sAMM-TK6/SOLEX');
      }
    });
  });
  ...
  describe('swaps fees corect calculate and transfer to feesVault for stable pairs', async () => {
    it('case protocol fee 0%, pairStable pair, stable fee 0.1% (MAX_FEE)', async () => {
      ...
      expect(await deployed.solex.balanceOf(await pairStable.communityVault())).to.be.eq(ZERO);

      await deployed.solex.transfer(pairStable.target, ONE_ETHER);

      await tokenTK6.mint(pairStable.target, 1e6);

      await pairStable.mint(signers.deployer.address);

      if ((await pairStable.token0()) == deployed.solex.target) {
        await deployed.solex.transfer(pairStable.target, ethers.parseEther('0.12'));
        ...
        expect(await deployed.solex.balanceOf(await pairStable.fees())).to.be.eq(calcFee);
    ...
    it('case protocol fee 100%, pairStable pair, custom fee 5% (MAX_FEE)', async () => {
      ...
      if ((await pairStable.token0()) == deployed.solex.target) {
        ...
        expect(await deployed.solex.balanceOf(await pairStable.fees())).to.be.eq(ZERO);

        expect(await deployed.solex.balanceOf(await pairStable.communityVault())).to.be.eq(calcFee);
      } else {
        await deployed.solex.transfer(pairStable.target, ethers.parseEther('0.12'));
    ...
    it('case protocol fee 80%, pairStable, custom fee 0.8%', async () => {
      ...
      if ((await pairStable.token0()) == deployed.solex.target) {
        ...
        expect(await deployed.solex.balanceOf(await pairStable.fees())).to.be.eq(calcFeeToFees);
