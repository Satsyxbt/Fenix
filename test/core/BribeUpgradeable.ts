...
describe('VoterUpgradeable Contract', function () {
  ...
  before(async function () {
    ...
    testBribe = BribeUpgradeable.attach(await testGauge.internal_bribe()) as BribeUpgradeable;
  ...
  describe('Deposit & Withdraw', async () => {
    describe('Deposit', async () => {
      ...
      it('GENERAL TEST simulation of corect deposits', async () => {
        ...
        // FIRST USER VOTE
        await voterInstance.connect(otheruser).vote(tokenId, [await testGauge.TOKEN()], [1]);
        await mine(1);
        ...
        await voterInstance.connect(otheruser2).vote(tokenId2, [await testGauge.TOKEN()], [1]);
        await mine(1);
