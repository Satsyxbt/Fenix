...
describe('BlastERC20RebasingManage Contract', function () {
  ...
  describe('#claim', async () => {
    it('Should fail if user not pass access check', async () => {
      await expect(rebasingManage.connect(other).claim(wethRebasing.target, other.address, 1)).to.be.revertedWith(ERRORS.Ownable.NotOwner);
    });
    ...
    it('Should success claim and return clamabl amount if balance not zero ', async () => {
      ...
      await setBalance(await wethRebasing.getAddress(), (await ethers.provider.getBalance(wethRebasing.target)) + ONE_ETHER * BigInt(100));
