...
describe('BribeFactoryUpgradeable Contract', function () {
  ...
  async function createBribe(owner: string, token0: string, token1: string, typeStr: string) {
    await bribeFactoryUpgradeableProxy.createBribe(token0, token1, typeStr);
    return bribeUpgradeableFactory.attach(await bribeFactoryUpgradeableProxy.last_bribe()) as BribeUpgradeable;
  }
